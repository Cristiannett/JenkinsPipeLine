pipeline {
    ...
    stages {
        stage('Stage1') {
            steps {
                script {
                    script1Result = bat(call "jenkinsScripts/stage1.js ${params.name}", returnStdout: true).trim()
                }
            }
        }
        stage('Stage2') {
            steps {
                script {
                    script2Result = bat(call "jenkinsScripts/stage2.js ${params.surname}", returnStdout: true).trim()
                }
            }
        }
        stage('Stage3') {
